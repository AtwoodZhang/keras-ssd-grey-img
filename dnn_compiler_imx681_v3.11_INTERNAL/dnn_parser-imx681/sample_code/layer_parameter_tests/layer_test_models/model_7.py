...
class LayerTest(nn.Module):
    def __init__(self, config, debug=False):
        ...
        if self.quantize:
            ...
            self.quant_6 = torch.quantization.QuantStub()
            self.quant_7 = torch.quantization.QuantStub()
    ...
    def forward(self, x):
        ...
        # Depthwise Conv2D
        x1 = self.layer_00(x) 
        activations['OP_0_DEPTHWISECONV2D'] = x1

        # Depthwise Conv2D
        x2 = self.layer_01(x)
        activations['OP_1_DEPTHWISECONV2D'] = x2

        x1 = self.dequant(x1)
        x2 = self.dequant(x2)
        
        # Sub
        x3_sub = torch.sub(x1, x2)
        

        x1 = self.quant_2(x1)
        x3_sub = self.quant_3(x3_sub)
        activations['OP_2_ADDSUB'] = x3_sub
        
        # Firmwarte uses  a CONCATENATE operation to implement a Pytorch dequantize->quantize if it is needed
        # x1 = self.dequant(x1)
        # x1 = self.quant_2(x1)

        # Multiply
        x = self.mul_1.mul(x1, x3_sub)
        activations['OP_4_MULTIPLY'] = x
        ...
        x = self.quant_4(x)
        activations['OP_5_MULTIPLY'] = x

        # Maxpool
        x = self.layer_05(x)
        activations['OP_6_MAXPOOL'] = x
        ...
        x = torch.mul(x, self.xs_2)
        
        x = self.quant_5(x)
        activations['OP_8_MULTIPLY'] = x
        
        x = self.dequant(x)
        x = torch.add(x, self.xs_3)
        

        x = self.quant_6(x)
        activations['OP_9_ADDSUB'] = x
        
        x = torch.reshape(x, (x.shape[0], 252))
        activations['OP_10_RESHAPE'] = x

        # Dense
        x = self.layer_10(x)
        activations['OP_11_FULLYCONNECTED'] = x
        x = self.dequant(x)
        # Softmax
        x = F.softmax(x, dim=1)

        # x_softmax = self.quant_7(x)
        # activations['OP_12_SOFTMAX'] = x_softmax
        activations['OP_12_SOFTMAX'] = x
