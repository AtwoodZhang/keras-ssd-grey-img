...
class LayerTest(nn.Module):
    ...
    def forward(self, x):
        ...
        # Relu
        x = self.layer_05(x)
        # activations['layer_05'] = x

        activations['OP_4_DEPTHWISECONV2D'] = x

        # Maxpool
        x1 = self.layer_06(x)
        activations['OP_5_MAXPOOL'] = x1

        # conv2d
        x2 = self.layer_07(x1)
        activations['OP_6_CONV2D'] = x2

        # concat
        x = self.concat_1.cat((x1, x2), dim=1)
        activations['OP_7_CONCATENATE'] = x

        # reshape
        
        x = torch.reshape(x,(4,144))
        activations['OP_8_RESHAPE'] = x

        # FC
        x = self.layer_10(x)
        activations['OP_9_FULLYCONNECTED'] = x
        ...
        # activations['OP_13_SOFTMAX'] = x_softmax
