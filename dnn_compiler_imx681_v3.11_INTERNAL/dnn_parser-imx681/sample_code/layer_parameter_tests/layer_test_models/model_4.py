...
class LayerTest(nn.Module):
    def __init__(self, config, debug=False):
        ...
        if self.quantize:
            ...
            self.quant_add_scalar = torch.quantization.QuantStub()
            self.quant_add_list = torch.quantization.QuantStub()
        ...
        # self.zz = torch.zeros([config["training"]["batch_size"], config["model"]["layer_16"]["out_channels"], 3, 4], device=cuda)
        devices = config["training"]["devices"]
  

        self.device = torch.device("cuda:"+str(devices))
    ...
    def forward(self, x):
        ...
        # Depthwise Conv2D
        x = self.layer_06(x)
        activations['OP_6_DEPTHWISECONV2D'] = x

        # Depthwise Conv2D
        x = self.layer_07(x)
        activations['OP_7_DEPTHWISECONV2D'] = x

        # Depthwise Conv2D, RELU
        x = self.layer_08(x)
        activations['OP_8_DEPTHWISECONV2D'] = x

        # Depthwise Conv2D, RELU
        x = self.layer_09(x)
        activations['OP_9_DEPTHWISECONV2D'] = x
