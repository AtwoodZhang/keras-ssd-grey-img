...
class SSDDatasets(keras.utils.Sequence):
    ...
    def __getitem__(self ,index):
        ...
        for i in range(index * self.batch_size, (index + 1) * self.batch_size): # (0,16)
            ...
            if len(box) != 0:
                ...
                # 进行归一化，调整到0~1之间
                boxes[:,[0,2]] = boxes[:,[0,2]]/(np.array(self.input_shape[1],np.float32))  # /160
                boxes[:,[1,3]] = boxes[:,[1,3]]/(np.array(self.input_shape[0],np.float32))  # /120
                # 对真实框的种类进行one hot处理
        ...
        # print(image_data)
        # b = Image.fromarray(image_data[0])
    ...
    def assign_boxes(self, boxes): # boxes=[[0.375, 0.25, 0.59, 0.59, 0, 1.],[0.33, 0.55, 0.518, 0.899, 0, 1.]]
        ...
        # 将xywh 转换成y_center, x_center, h, w
