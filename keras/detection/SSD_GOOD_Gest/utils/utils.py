...
def cvtColor(image):
    ...
    elif len(np.shape(image)) == 3 and np.shape(image)[2] == 3:
        image = image.convert('L')
    elif len(np.shape(image)) == 3 and np.shape(image)[2] == 1:
        image = image.convert('L')
    return image


def cvtColor2(image2):
    if len(np.shape(image2)) == 3 and np.shape(image2)[2] == 3:
        return image2
    else:
