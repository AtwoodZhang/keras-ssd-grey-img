...
class SSDDatasets(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for i in range(index * self.batch_size, (index + 1) * self.batch_size):
            ...
            box_data.append(box)
        tmp_img_data = (np.array(image_data, np.float32) / 127.5 - 1.)  # 归一化成-1，1
        tmp_box_data = np.array(box_data)
        return tmp_img_data, tmp_box_data
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        image = cvtColor(image)  # (120, 160, 3)  --> (120, 160)
        # ------------------------------#
        #   获得图像的高宽与目标高宽
        # ------------------------------#
        iw = image.size[0]  # iw, ih = image.size
        ih = image.size[1]
    ...
    def encode_box(self, box, return_iou=True, variances=[0.1, 0.1, 0.2, 0.2]):
        ...
        # print("assgned anchors:", assigned_anchors)
        # ---------------------------------------------#
