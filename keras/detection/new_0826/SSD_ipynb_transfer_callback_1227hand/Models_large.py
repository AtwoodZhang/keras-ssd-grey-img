...
def mobilenet(inputs_tensor):
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 30 * 40 * 8  change
    x = Conv2D(16, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer5')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    # Block 1~3  
    x = _depthwise_conv_block(x, 16, 1, block_id=1)  # --> 30 * 40 * 8
    x = _depthwise_conv_block(x, 16, 1, block_id=2)  # --> 30 * 40 * 8
    x = _depthwise_conv_block(x, 16, 1, block_id=3)  # --> 30 * 40 * 8             

    # Conv_Depth_Conv_Depth_Conv
    # --> 30 * 40 * 16
    x = Conv2D(24, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer6')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 30 * 40 * 16
    x = Conv2D(32, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer8')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 15 * 20 * 24 
    x = Conv2D(64, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer10')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    # Block 4~6
    x = _depthwise_conv_block(x, 64, 1, block_id=4)  # --> 15 * 20 * 24
    x = _depthwise_conv_block(x, 64, 1, block_id=5)  # --> 15 * 20 * 24
    x = _depthwise_conv_block(x, 64, 1, block_id=6)  # --> 15 * 20 * 24

    # --> 15 * 20 * 48
    x = Conv2D(64, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer11')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 8 * 10 * 40
    x = Conv2D(64, (1, 1), padding='same', use_bias=True, strides=(2, 2), name='Conv2D_layer15')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
        x = tf.nn.relu(x)  # tf 1.13.2
    # block 7_1 ~7_2
    x = _depthwise_conv_block(x, 64, 1, block_id=7)  # --> 8 * 10 * 40
    x = _depthwise_conv_block(x, 64, 1, block_id=8)  # --> 8 * 10 * 40
    # --> 8 * 10 * 80
    x = Conv2D(128, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer16')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 8 * 10 * 80
    x = Conv2D(128, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer18')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 4 * 5 * 80
    x = Conv2D(128, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer20')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    # --> 2 * 3 * 64
    x = Conv2D(160, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer22')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
    ...
    try:
        ...
        x = tf.nn.relu(x)  # tf 1.13.2
    x = Conv2D(160, (1, 1), padding='same', use_bias=True, strides=(1, 1), name='Conv2D_layer24')(x)
    x = BatchNormalization()(x)
    try:
        x = KL.ReLU(max_value=6.)(x)
    except:
...
def SSD300(input_shape, num_classes=2):
    ...
    #    layer1-bbox, 4是x, y, h, w的调整
    net['split_layer1_loc_Dep'] = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=1, strides=(1, 1), use_bias=True, name='DepthwiseConv2D_loc_DD1_1')(net['split_layer1'])
    net['split_layer1_loc_Dep'] = BatchNormalization()(net['split_layer1_loc_Dep'])
    try:
        net['split_layer1_loc_Dep'] = KL.ReLU(max_value=6.)(net['split_layer1_loc_Dep'])
    except:
        net['split_layer1_loc_Dep'] = tf.nn.relu(net['split_layer1_loc_Dep'])
    ...
    #    layer2-bbox, 4是x, y, h, w的调整
    net['split_layer2_loc_Dep'] = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=1, strides=(1, 1), use_bias=True, name='DepthwiseConv2D_loc_DD2_1')(net['split_layer2'])
    net['split_layer2_loc_Dep'] = BatchNormalization()(net['split_layer2_loc_Dep'])
    try:
        net['split_layer2_loc_Dep'] = KL.ReLU(max_value=6.)(net['split_layer2_loc_Dep'])
    except:
        net['split_layer2_loc_Dep'] = tf.nn.relu(net['split_layer2_loc_Dep'])
    ...
    #    layer3-bbox, 4是x, y, h, w的调整
    net['split_layer3_loc_Dep'] = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=1, strides=(1, 1), use_bias=True, name='DepthwiseConv2D_loc_DD3_1')(net['split_layer3'])
    net['split_layer3_loc_Dep'] = BatchNormalization()(net['split_layer3_loc_Dep'])
    try:
        net['split_layer3_loc_Dep'] = KL.ReLU(max_value=6.)(net['split_layer3_loc_Dep'])
    except:
        net['split_layer3_loc_Dep'] = tf.nn.relu(net['split_layer3_loc_Dep'])
    ...
    #    layer4-bbox, 4是x, y, h, w的调整
    net['split_layer4_loc_Dep'] = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=1, strides=(1, 1), use_bias=True, name='DepthwiseConv2D_loc_DD4_1')(net['split_layer4'])
    net['split_layer4_loc_Dep'] = BatchNormalization()(net['split_layer4_loc_Dep'])
    try:
        net['split_layer4_loc_Dep'] = KL.ReLU(max_value=6.)(net['split_layer4_loc_Dep'])
    except:
        net['split_layer4_loc_Dep'] = tf.nn.relu(net['split_layer4_loc_Dep'])
    ...
    #    layer5-bbox, 4是x, y, h, w的调整
    net['split_layer5_loc_Dep'] = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=1, strides=(1, 1), use_bias=True, name='DepthwiseConv2D_loc_DD5_1')(net['split_layer5'])
    net['split_layer5_loc_Dep'] = BatchNormalization()(net['split_layer5_loc_Dep'])
    try:
        net['split_layer5_loc_Dep'] = KL.ReLU(max_value=6.)(net['split_layer5_loc_Dep'])
    except:
        net['split_layer5_loc_Dep'] = tf.nn.relu(net['split_layer5_loc_Dep'])
