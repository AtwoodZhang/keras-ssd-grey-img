...
class SSDDatasets(keras.utils.Sequence):
    ...
    def __getitem__(self ,index):
        ...
        return image_data, [box_data, box_data]
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, random=True):  # jitter颜色相关
        img_root = "datasets/VOCdevkit/VOC2007/JPEGImages"
        ...
        if len(box) > 0:
            ...
            box = box[np.logical_and(box_w > 1, box_h > 1)]  # discard invalid box
                ...
                #  Gauss
        gauss = self.rand() < .1
        if gauss: image = image.filter(ImageFilter.GaussianBlur(radius=3))

        # brightness
        bright = self.rand() < .05
        enhancer = ImageEnhance.Brightness(image)
        ratio = self.rand()/10
        if bright: image = enhancer.enhance(ratio)
    ...
    def assign_boxes(self, boxes): # boxes=[[0.375, 0.25, 0.59, 0.59, 0, 1.],[0.33, 0.55, 0.518, 0.899, 0, 1.]]
        ...
        for i in range(self.num_anchors):
            if assignment[i, -1] == 1:  # 只转换非背景框
                assignment[i, :4] = convert_box_format(assignment[i, :4])
        return assignment
