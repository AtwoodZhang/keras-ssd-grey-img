...
class BBoxUtility(object):
    def __init__(self, num_classes, nms_thresh=0.1, top_k=300):
    ...
    def decode_box(self, predictions, anchors, image_shape, input_shape, letterbox_image, variances = [0.1, 0.1, 0.2, 0.2], confidence=0.5):
        mbox_loc        = predictions[:, :, :4] #yxhw
        mbox_loc[:, :, [0, 1]] = mbox_loc[:, :, [1, 0]] #xywh
        mbox_loc[:, :, [2, 3]] = mbox_loc[:, :, [3, 2]]
        ...
        for i in range(len(mbox_loc)):
            ...
            for c in range(1, self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    # good_boxes  = boxes_to_process
                    # confs       = confs_to_process[:, None]
                    # labels      = (c - 1) * np.ones((boxes_to_process.shape[0], 1))
