...
def train(loader, model, criterion, optimizer, epoch, device, fabric, debug_step=100):
    model.train()
    running_loss = 0.0
    for batch_idx, data in enumerate(loader):
        ...
        optimizer.zero_grad()
        pred = model(images)  # forward padd
        ...
        if batch_idx % debug_step == 0 and batch_idx > 0:
            ...
            logging.info(
                f"Epoch: {epoch}, Step: {batch_idx}, " + f"Average Loss: {avg_loss:.4f}" + f"time cost: {t4-t3: .4f}")
            running_loss = 0.0
    return avg_loss
...
def main():
    ...
    for epoch in range(args.num_epochs):
        t1=time.time()
        train_loss = train(train_loader, model, criterion, optimizer, epoch, DEVICE, fabric)
        ...
        logging.info(
            ...
            f"time cost: {t2-t1:.4f}"
        )
        print("one epoch cost time: ", t2-t1)
