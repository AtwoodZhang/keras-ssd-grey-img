...
def create_mobilenetv2_ssd_lite(num_classes, width_mult=1.0, use_batch_norm=True, onnx_compatible=False, is_test=False):
    ...
    source_layer_indexes = [14]
    #TODO:681 sample model last 4 layers
    extras = ModuleList([
        InvertedResidual(80, 80, stride=1, expand_ratio=1),
        InvertedResidual(80, 80, stride=2, expand_ratio=1),
        InvertedResidual(80, 64, stride=2, expand_ratio=1),
        InvertedResidual(64, 64, stride=3, expand_ratio=1)
    ])

    regression_headers = ModuleList([
        SeperableConv2d(in_channels=round(64 * width_mult), out_channels=2 * 4,
                        kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=80, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=80, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=64, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
        SeperableConv2d(in_channels=64, out_channels=6 * 4, kernel_size=3, padding=1, onnx_compatible=False),
